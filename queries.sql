...
from analyst a
where not exists(select id, instant  -- Every incidents on 'B-789'
                 from incident
                 where id = 'B-789'
                     except
                 select id, instant -- every incident analysed by the analyst in question
                 from analyses an
...
WHERE (sname,saddress) NOT IN(
    SELECT sname, saddress -- supervisors from substations south of Rio Maior
    FROM substation
...
GROUP BY id
HAVING COUNT(id) <= ALL( -- Elements that verify this condition have the smallest amount of incidents
        SELECT COUNT(i2.id)
        FROM  incident i2 
        GROUP BY i2.id);
